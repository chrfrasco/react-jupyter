...
class DisplayData extends Component {
  render() {
    ...
    if (!format) {
      return <div className="empty-output" />
    }

    const data = raw[format] || raw.data[format]

    if (['text', 'text/plain'].includes(format)) {
      return <Code className="text-output" string={joinText(data)} />
    }
    ...
    if (['text/markdown'].includes(format)) {
      return <Markdown className="html-output" string={joinText(data)} />
    }
    ...
    if (['svg', 'text/svg+xml'].includes(format)) {
      return <div className="svg-output">{joinText(data)}</div>
    }
